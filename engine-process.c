...
UINT make_colour(UINT original, float directional_ratio, float distance) {
	int ro = (original & 0x0000FF);
	int go = (original & 0x00FF00) >> 8;
	int bo = (original & 0xFF0000) >> 16;

	// Set ambient
	UINT r_out = ambient_light.r * ro;
	UINT g_out = ambient_light.g * go;
	UINT b_out = ambient_light.b * bo;
